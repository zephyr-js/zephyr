...
export const welcome = [
  ...
  'Time to build a new API.',
  'Time to build a faster API.',
  'Time to build a sweet new API.',
...
export function getVersion() {
  return new Promise<string>((resolve) => {
    ...
    get('https://registry.npmjs.org/@zephyr-js/core/latest', (res) => {
      let body = '';
...
export async function typescriptByDefault() {
  await info(
    'Cool!',
    'Zephyr comes with TypeScript support enabled by default.',
  );
  console.log(
    `${' '.repeat(3)}${color.dim(
